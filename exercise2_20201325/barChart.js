...
d3.csv("data.csv").then(data => {
  ...
  legend.on("click", function(event, category) {
    appState.updateSelectedProduct(category);  // 선택한 카테고리를 상태에 저장
  });
...
updateBarChart = function(appState) {
  // 선택된 시간범위로 데이터 필터링
  const timeFiltered = appState.timeRange
    ? nested.filter(d => {
        const date = new Date(d.month + "-01"); // month = "2023-01"
  ...
  const newStack = d3.stack().keys(activeCats);
  const newSeries = newStack(timeFiltered);

  y.domain([0, d3.max(timeFiltered, d => d3.sum(activeCats, k => d[k] || 0))]);
  g.select(".y-axis").transition().call(d3.axisLeft(y));
  ...
  // 선택된 row에 해당하는 월, 카테고리 강조 (테두리 표시)
  if (appState.selectedRows && appState.selectedRows.length > 0) {
    const selected = appState.selectedRows[0];
    const selectedMonth = d3.timeFormat("%Y-%m")(d3.timeParse("%d-%b-%y")(selected.Date));
    const selectedCategory = selected.Product.split(" ")[0];
